...
class ProbabilityEstimator:
    ...
    def __init__(self, p, scorefun, proposal, default_val, save_trace=False, initial_weights=None, n_samples = 1000000):
    ...
    def __get_score_trace(self, n_samples=1000000):
        ...
        with pm.Model() as m:
            s = pm.WeightedScoreDistribution('S', scorer=self.scorefun, weighting=self.weights,
                                             cat=True,
    ...
    def __estimate_masked(self, mask, score_trace):
        prob = estimates._estimate_numenator(score_trace, mask, self.weights) / estimates._estimate_denominator(
            score_trace, mask, self.weights)
        self.prob = prob
        self.var = estimates.varianceOBM(score_trace, self.weights, mask)
